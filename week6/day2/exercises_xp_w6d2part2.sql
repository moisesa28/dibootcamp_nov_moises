...
SELECT DISTINCT create_date FROM customer; -- the DISTINCT makes sure that there are no duplicates
...
SELECT film_id,title,description,length,rental_rate FROM film WHERE title = 'Dinosaur';
...
--11. Not satisfied with the results. Write a query which will find the 
-- next 10 cheapest movies. Bonus: Try to not use LIMIT.
SELECT * FROM film WHERE rental_rate<2
...
SELECT title FROM film where film_id NOT IN(SELECT film_id FROM inventory); --the NOT IN excludes any item not in the table
...
CONCAT(customer.first_name, ' ', customer.last_name) AS full_name, -- concatenate to get one column for full name
payment.amount,
payment.payment_date
FROM customer
INNER JOIN payment ON customer.customer_id = payment.staff_id ORDER BY staff_id ASC